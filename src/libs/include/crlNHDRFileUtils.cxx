...
namespace crl {
	...
	*************************************************************************************************/
	 ...
	 * ### exception	itk	:ExceptionObject	Thrown when error. 
	*************************************************************************************************/
	void NHDR_CopyAndModifyHeader(const string& refHdr, const string& outputHdr, const std::vector<string>& filenames, const std::vector<vnl_vector_fixed< double,3 > >& gvectors, bool useCompression )
	{
		...
		while ( (!ifile.eof()) && next)
		{
			...
			else if ( line.find("encoding:")!=string::npos )
			{
				if ( useCompression ) {
					ofile << "encoding: gzip" << std::endl;
					ofile << "line skip: 13" << std::endl;
				}
				else
					ofile << "encoding: raw" << std::endl;
			}
			else if  ( line.find("line skip:")!=string::npos )
			{
				// do nothing. was defined by the encoding
			}
			else
				ofile << szBuffer << endl;
		}
		...
		if ( modifyGradients )
		{
			...
			{
				const vnl_vector_fixed<double,3>& v = gvectors[g];
	...
	*************************************************************************************************/
	void NHDR_CopyAndModifyHeader(const string& refHdr, const string& outputHdr, const std::vector<string>& filenames, bool useCompression)
	{
		std::vector< vnl_vector_fixed< double,3 > > emptyVecs;
		NHDR_CopyAndModifyHeader(refHdr, outputHdr, filenames, emptyVecs, useCompression);
	}
