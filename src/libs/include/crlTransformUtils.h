...
namespace crl {
		...
		LoadAnyTransformAsAffine ( const std::string& filename )
	{
		...
		typename itk::AffineTransform< TScalarType, NDimensions >::Pointer finalAffineTransform = itk::AffineTransform< TScalarType, NDimensions >::New();
		finalAffineTransform->SetIdentity();

		// Check that only one transform
		if (transforms->size() == 0 )
			throw itk::ExceptionObject(__FILE__,__LINE__,"Error. Cannot find any transform in this file.");
		else if ( transforms->size() == 1 ) 
			finalAffineTransform = ConvertAnyTransformToAffine<TScalarType, NDimensions> (*it);
		else
		{
			std::cout << "  Compose transforms... (" << transforms->size() <<" transforms)" << std::endl;
			for (it=transforms->begin(); it != transforms->end(); it++) 
			{
				typename itk::AffineTransform< TScalarType, NDimensions >::Pointer transform = ConvertAnyTransformToAffine<TScalarType, NDimensions> (*it);
				finalAffineTransform->Compose(transform, true);
			} 
		}

		// to avoid compilation warning/error
		return finalAffineTransform;
	}
