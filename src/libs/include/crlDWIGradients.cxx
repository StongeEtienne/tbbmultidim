...
void DWIGradients::ReadFromFile(const std::string& fileName ) 
{
	//------------------------------------------------
	// Open the file
	//------------------------------------------------
	...
	if ( !infile.is_open() )
		throw itk::ExceptionObject(__FILE__,__LINE__,"Error. Cannot open the file <"+fileName+">", "");

	std::cout<<"- Reading gradient file <" << fileName << ">..." << std::endl;

	//------------------------------------------------
	// Iterates on all lines
	//------------------------------------------------
	int nbB0=0;
	int nbTotal=0;
	while (getline(infile, line, '\n'))
	{
		...
		if ( sscanf(line.c_str(), "Vector[%d] = ( %g, %g, %g )", &num, &x, &y, &z )==4 )
		{
			...
			m_DiffusionVectors->InsertElement( m_DiffusionVectors->Size(), vect3d );
			nbTotal++;
			if ( vect3d.two_norm()==0 ) nbB0++;
		}
	}

	std::cout<<"  Found " << nbB0 << " b=0, "<< nbTotal-nbB0 <<" b>0 gradients" << std::endl;
...
void DWIGradients::AddOneShell(int nbDirections, double _B0)
{
	...
	/*GradientDirectionType vect3d, vref;

	vect3d[0] = *p; vect3d[1] = *(p+1); vect3d[2] = *(p+2);
	vect3d = vect3d*(sqrt(_B0/m_NominalB0));

	vref[0]=1.0/sqrt(3.0); vref[1]=1.0/sqrt(3.0);  vref[2]=1.0/sqrt(3.0); 
	vref = vref*(sqrt(_B0/m_NominalB0));

	double theta = -::angle<double>(vref,vect3d);
	GradientDirectionType u = vnl_cross_3d( vref, vect3d );
	u = u.normalize();
	...
	std::cout<<"WARNING USING ROTATION IN AddOneShell"<<std::endl;
*/
