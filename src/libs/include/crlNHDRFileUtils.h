...
namespace crl {
	extern void NHDR_CopyAndModifyHeader(const std::string& refHdr, const std::string& outputHdr, const std::vector<std::string>& filenames, const std::vector<vnl_vector_fixed< double,3 > >& gvectors, bool useCompression );
	extern void NHDR_CopyAndModifyHeader(const std::string& refHdr, const std::string& outputHdr, const std::vector<std::string>& filenames, bool useCompression);
	...
	*************************************************************************************************/
                                                                ...
                                                                bool useCompression=false )
	{
		...
		//-------------------------------------------------------
		NHDR_CopyAndModifyHeader(refNHDRHeader, fnOutput.getCompleteFilePath(), createdFileNames, gvecs, useCompression );
	...
	*************************************************************************************************/
								...
								const std::string& measurementFrame="" )
	{
		...
		ssOrigin << std::fixed << std::showpoint << std::setprecision(8) << "space origin: (";		// Need std::fixed << std::showpoint to be sure the precision is after the '.'
		...
		ssDir << std::fixed << std::showpoint << std::setprecision(8) << "space directions:";
		...
		if ( useCompression ) {
			nhdrFile << "encoding: gzip" << std::endl;
			nhdrFile << "line skip: 13" << std::endl;
		}
		else nhdrFile << "encoding: raw" << std::endl;
