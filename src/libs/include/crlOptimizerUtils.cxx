...
namespace crl {
	...
	const unsigned int OptimizerUtils::OPTIMIZER_NB = 12;
	...
	*************************************************************************************************/
	OptimizerUtils::GenericOptimizerType::Pointer OptimizerUtils::CreateOptimizer(OptimizerIdType optimizerId, int nbMaxIter, bool maximize )
	{
		...
		if ( optimizerId==OPTIMIZER_LBFGS )
		{
			...
			LBFGSoptimizer->SetMaximize(maximize);
		...
		else if ( optimizerId==OPTIMIZER_FRPR )
		{
			...
			FRPRoptimizer->SetMaximize(maximize);
		...
		else if ( optimizerId==OPTIMIZER_CONJGRAD )
		{
			...
			CONJGRADoptimizer->SetMaximize(maximize);
		...
		else if ( optimizerId==OPTIMIZER_POWELL)
		{
			...
			POWELLoptimizer->SetMaximize(maximize);
		...
		else if ( optimizerId==OPTIMIZER_NEWUOA )
		{
			...
			NLOPTOptimiz->SetMaximize(maximize);
		}
		...
		// COBYLA - from NLOPT
		//---------------------------------------------------------------
		else if ( optimizerId==OPTIMIZER_COBYLA )
		{
			typedef itk::NLOPTOptimizers NLOPTOptimizerType;
			NLOPTOptimizerType::Pointer NLOPTOptimiz = NLOPTOptimizerType::New();
			optimizer = NLOPTOptimiz;
			NLOPTOptimiz->SetAlgorithm(NLOPTOptimizerType::NLOPT_LN_COBYLA );
			NLOPTOptimiz->SetMaximize(maximize);
		}
		...
		// DIRECTL - from NLOPT
		//---------------------------------------------------------------
		else if ( optimizerId==OPTIMIZER_DIRECTL )
		{
			typedef itk::NLOPTOptimizers NLOPTOptimizerType;
			NLOPTOptimizerType::Pointer NLOPTOptimiz = NLOPTOptimizerType::New();
			optimizer = NLOPTOptimiz;
			NLOPTOptimiz->SetAlgorithm(NLOPTOptimizerType::NLOPT_GN_DIRECT_L_RAND );
			NLOPTOptimiz->SetMaximize(maximize);
		} 
		...
		// PRAXIS - from NLOPT
		//---------------------------------------------------------------
		else if ( optimizerId==OPTIMIZER_PRAXIS )
		{
			typedef itk::NLOPTOptimizers NLOPTOptimizerType;
			NLOPTOptimizerType::Pointer NLOPTOptimiz = NLOPTOptimizerType::New();
			optimizer = NLOPTOptimiz;
			NLOPTOptimiz->SetAlgorithm(NLOPTOptimizerType::NLOPT_LN_PRAXIS );
			NLOPTOptimiz->SetMaximize(maximize);
		} 

		//---------------------------------------------------------------
		// NELDER MEAD - from NLOPT
		//---------------------------------------------------------------
		else if ( optimizerId==OPTIMIZER_NELDERMEAD_SIMPLEX )
		{
			typedef itk::NLOPTOptimizers NLOPTOptimizerType;
			NLOPTOptimizerType::Pointer NLOPTOptimiz = NLOPTOptimizerType::New();
			optimizer = NLOPTOptimiz;
			NLOPTOptimiz->SetAlgorithm(NLOPTOptimizerType::NLOPT_LN_NELDERMEAD );
			NLOPTOptimiz->SetMaximize(maximize);
		} 
		//---------------------------------------------------------------
		// NELDER MEAD - from NLOPT
		//---------------------------------------------------------------
		else if ( optimizerId==OPTIMIZER_SUBPLEX )
		{
			typedef itk::NLOPTOptimizers NLOPTOptimizerType;
			NLOPTOptimizerType::Pointer NLOPTOptimiz = NLOPTOptimizerType::New();
			optimizer = NLOPTOptimiz;
			NLOPTOptimiz->SetAlgorithm(NLOPTOptimizerType::NLOPT_LN_SBPLX );
			NLOPTOptimiz->SetMaximize(maximize);
		} 
		...
		//else if ( optimizerId== )
		//{
		//	typedef itk::GradientDescentLineSearchOptimizerv4 GradientDescOptimizerType;
		//	GradientDescOptimizerType::Pointer opt = GradientDescOptimizerType::New();
		//	optimizer = opt;
		//}
		...
		//cannot work for vnl based optimizers. the cost function must be set before
	...
	*************************************************************************************************/
	void OptimizerUtils::SetOptimizerMaxNbIter(GenericOptimizerType* optimizer, int nbMaxIter)
	{
		...
		//---------------------------------------------------------------
		typedef itk::AmoebaOptimizer 		AmoebaOptimizerType;
		AmoebaOptimizerType *amoebagrad = dynamic_cast<AmoebaOptimizerType *>(optimizer);
		if ( amoebagrad!=NULL )
		{
			amoebagrad->SetMaximumNumberOfIterations( nbMaxIter );
			return;
		}
	...
	*************************************************************************************************/
		 ...
		 double stepLength, double stepTol, double valTol, double gradTol, int maxNbIter, bool maximize )
	{
		...
		if ( lbfgs!=NULL )
		{
			...
			if ( vnlOptimizer==NULL ) throw itk::ExceptionObject(__FILE__,__LINE__, "Error in OptimizerUtils::SetOptimizerParameters (vnlOptimizer==NULL). The cost function should be set before calling SetOptimizerParameters ", "" );
		...
		if ( amoebagrad!=NULL )
		{
			amoebagrad->SetParametersConvergenceTolerance(stepTol);
			amoebagrad->SetFunctionConvergenceTolerance(valTol);

			if ( !maximize ) amoebagrad->MinimizeOn();
			else amoebagrad->MaximizeOff();
			return;
		}
