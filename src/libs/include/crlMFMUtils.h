...
namespace crl {
	...
	*************************************************************************************************/
	...
	void MFM_LoadTensors ( const std::vector<std::string> & filenames, std::vector<typename TensorImageType::Pointer>& tensorImages, bool verbose=true  )
	{
		tensorImages.clear();
		...
		{
			if ( verbose ) std::cout<<"  " << filenames[i].c_str() << std::endl;
