...
public:
    ...
    *************************************************************************************************/
	...
	void SetAdditionalHelp(const std::string& additionalHelp )
	{
		m_AdditionalHelp = additionalHelp;
	}

	const std::string& GetAdditionalHelp() const
	{
		return this->m_AdditionalHelp;
	}
	...
	void showProgNameAndVersion(TCLAP::CmdLineInterface& _cmd, std::ostream& c)
	{
		...
		c << "Boston Children's Hospital."<<std::endl;
		c << "  Computational Radiology Laboratory (CRL)."<<std::endl;
		if ( m_AuthorName!="") c<<"  ("<<m_AuthorName<<")"<<std::endl;
#if defined(__INTEL_COMPILER) || defined(__ICC)
		int majV= (int)__INTEL_COMPILER/100;
		int minV=__INTEL_COMPILER-majV*100;
                c << "* Compiled with Intel Compiler " << majV << "." << minV << std::endl;
#elif defined(_MSC_VER)
		int majV= (int)_MSC_VER/100;
		int minV=_MSC_VER-majV*100;
                c << "* Compiled with Microsoft Visual Studio " << majV << "." << minV << std::endl;
#elif defined(__GNUC__) || defined(__GNUG__)
		c << "* Compiled with GNU GCC/G++ " <<  __VERSION__ << std::endl;
#endif
		c << "=========================================================================="<<std::endl;
	...
	virtual void usage(TCLAP::CmdLineInterface& _cmd)
	{
		...
		if ( m_AdditionalHelp!="" )
			std::cout << m_AdditionalHelp.c_str() << std::endl<< std::endl;
