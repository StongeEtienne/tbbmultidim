...
#define cursor_savepos "\33[s"
#define cursor_restorepos "\33[u"
...
#ifdef WIN32
	int		g_InitYLine = 0;
	HANDLE  g_TaskProgressMutex = CreateMutex(NULL, FALSE, NULL);
#else
	pthread_mutex_t g_TaskProgressMutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
class TaskProgress::PrivateData
	...
	*************************************************************************************************/
	...
	{
		g_NbFinished = 0;		//don t init g_LastProcToCout here bcause will be called in multiple threads!
		d->m_TimeProbe.Start();
		ShowProgress();
	...
	*************************************************************************************************/
	void TaskProgress::Update()
	{
		...
		//if ( d->m_CurrentCounter>d->m_CounterMaxVal ) d->m_CurrentCounter = d->m_CounterMaxVal ;
	...
	*************************************************************************************************/
	...
	{
#ifdef WIN32
		WaitForSingleObject(g_TaskProgressMutex, INFINITE);
#else
		pthread_mutex_lock( &g_TaskProgressMutex );
#endif
		...
		if ( g_LastProcToCout==-1 )
		{
			...
			// Init all the lines
			//-------------------------------------
			for ( unsigned int i=0; i<(d->m_NbThreads) ; i++ ) std::cout<<d->m_NbThreads<<std::endl;
			flush(std::cout);
		...
		//-------------------------------------
		std::cout<<cursor_savepos; //"\e[s";
		std::cout<<cursor_up(d->m_NbThreads-d->m_ThreadID);
#endif
		...
		//-------------------------------------
		pos.X=0;
		pos.Y=g_InitYLine + d->m_NbThreads;
		SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), pos);
#else
		//-------------------------------------
		// Linux/OSX : restore previous cursor pos
		//-------------------------------------
		std::cout<<cursor_restorepos; //"\e[u";
#endif

#ifdef WIN32
		ReleaseMutex(g_TaskProgressMutex);
#else
		pthread_mutex_unlock( &g_TaskProgressMutex );
#endif
	}
