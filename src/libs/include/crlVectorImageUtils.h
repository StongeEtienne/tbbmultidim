...
namespace crl {
	...
	*************************************************************************************************/
	...
	template  < typename TPixel, unsigned int VImageDimension  >
	typename itk::Image< TPixel, VImageDimension >::Pointer ExtractComponentOfVectorImageFast( const typename itk::VectorImage<TPixel, VImageDimension>::Pointer &inputImage ,  int id )
	{
		typedef typename itk::VectorImage<TPixel, VImageDimension>	InputVectorImage;
		typedef typename itk::Image<TPixel, VImageDimension>		OutputOneImage;
		...
		//-------------------------------------------------
		typename InputVectorImage::SizeType s = inputImage->GetLargestPossibleRegion().GetSize();
		unsigned long int nVoxels = 1;
		for ( unsigned int i=0; i<VImageDimension; i++ )
			nVoxels = nVoxels*s[i];

		/*-------------------------------------------------
		Copy one image of the vector image to oneImage using direct memory access - faster than iterators
		/-------------------------------------------------*/
		typename InputVectorImage::InternalPixelType *p_in = inputImage->GetBufferPointer();
		typename OutputOneImage::InternalPixelType *p_out = oneImage->GetBufferPointer();
		p_in += id;
		int nComponents = inputImage->GetNumberOfComponentsPerPixel();
		
		for ( unsigned long int i=0; i<nVoxels ; i++ )
		{
			*p_out = *p_in;
			p_out++;
			p_in+=nComponents;
	...
	*************************************************************************************************/
	...
	/**********************************************************************************************//**
	 * \fn	template < typename TPixel, unsigned int VImageDimension > typename itk::VectorImage<TPixel, VImageDimension>::Pointer CreateVectorImage( typename itk::VectorImage<TPixel, VImageDimension>::Pointer &srcImage,
	 * 		std::vector<unsigned int> ids )
	 *
	 * \brief	Create a vector image of size  ids.size() and copy the values of each index of ids
	 * 			from the vector image srcImage.
	 *
	 * \author	Benoit Scherrer
	 * \date	April 2010
	 *
	 * \exception	itk::ExceptionObject	Thrown when error.
	 *
	 * \tparam	typename TPixel					Type of the typename t pixel.
	 * \tparam	unsigned int VImageDimension	Type of the unsigned int v image dimension.
	 * \tparam	typename						TPixel					Type of the typename t pixel.
	 * \tparam	unsigned						int VImageDimension	Type of the unsigned int v image
	 * 											dimension.
	 * \param	srcImage	Source vector image.
	 * \param	ids			The component indexes (from the source vector image).
	 *
	 * \return	The new vector image&lt;typename t pixel,unsigned int v image dimension &gt;
	 **************************************************************************************************/
	template  < typename TPixel, unsigned int VImageDimension  >
	typename itk::VectorImage<TPixel, VImageDimension>::Pointer ExtractVectorImageFromVectorImage( const typename itk::VectorImage<TPixel, VImageDimension>::Pointer &srcImage, std::vector<unsigned int> ids )
	{
		//-------------------------------------------------------
		// Defines some useful types
		//-------------------------------------------------------
		typedef typename itk::VectorImage<TPixel, VImageDimension>	VectorImageType;
		typedef typename itk::ImageRegionIterator<VectorImageType>	VecImageIteratorType;

		//-------------------------------------------------------
		// Check....
		//-------------------------------------------------------
		for ( unsigned int i=0; i<ids.size(); i++ )
			if ( ids[i]>=srcImage->GetNumberOfComponentsPerPixel() )
				throw itk::ExceptionObject("ExtractVectorImageFromVectorImage: an index is out-of-bound. ");

		//-------------------------------------------------------
		// Create the output image
		//-------------------------------------------------------
		typename VectorImageType::Pointer outputImage = VectorImageType::New();
		outputImage->CopyInformation(srcImage);
		outputImage->SetRegions(srcImage->GetLargestPossibleRegion().GetSize());
		outputImage->SetOrigin(srcImage->GetOrigin());
		outputImage->SetSpacing(srcImage->GetSpacing());
		outputImage->SetDirection(srcImage->GetDirection());
		outputImage->SetNumberOfComponentsPerPixel(ids.size());
		outputImage->Allocate();

		//-------------------------------------------------------
		// Creates the iterators
		//-------------------------------------------------------
		VecImageIteratorType outItr( outputImage, outputImage->GetLargestPossibleRegion() );
		VecImageIteratorType inItr ( srcImage, srcImage->GetLargestPossibleRegion()); 
		inItr.GoToBegin(); outItr.GoToBegin();

		typename VectorImageType::PixelType inP(srcImage->GetNumberOfComponentsPerPixel() ) ;
		typename VectorImageType::PixelType outP(outputImage->GetNumberOfComponentsPerPixel() ) ;

		//-------------------------------------------------------
		// Copy all pixels of the src image
		//-------------------------------------------------------
		for ( inItr.GoToBegin(), outItr.GoToBegin() ; !outItr.IsAtEnd() ; ++outItr, ++inItr )
		{
			inP = inItr.Get();
		...
		/*typedef typename itk::ImageFileWriter< VectorImageType > WriterType;
		typename WriterType::Pointer writer = WriterType::New();
		writer->SetInput(outputImage);
		writer->SetFileName( "testben.nii.gz" );
...
#else
		typedef itk::ComposeImageFilter< OneImageType > ImageToVectorImageFilterType;
#endif
