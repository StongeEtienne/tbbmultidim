...
namespace itk
{
	...
	*************************************************************************************************/
	double NLOPTOptimizers::NloptFunctionWrapper(unsigned n, const double *x, double *grad, void *data)
	{
		...
		if ( grad!=NULL )
		{
	        ...
	        optimizer->m_NumberOfEvaluations++;
	...
	*************************************************************************************************/
	void NLOPTOptimizers::StartOptimization()
	{
		...
		if ( lb!=NULL ) nlopt_set_lower_bounds(m_NloptOptions, lb);
		if ( ub!=NULL ) nlopt_set_upper_bounds(m_NloptOptions, ub);
		if ( m_Maximize )
		...
		case NLOPT_FORCED_STOP:
			...
			ss<< std::endl<<"After scaling" << std::endl<< "In x:";
			for ( unsigned int i=0; i<n ; i++ ) ss <<" " << x[i] ;
			ss << std::endl;

			if ( m_LowerBoundParameters.GetSize()!=0 )
			{
				ss<<"L B:";
				for ( unsigned int i=0; i<n ; i++ ) ss <<" " << lb[i] ;
				ss << std::endl;
			}
			...
			{
				ss<<"U B:";
				for ( unsigned int i=0; i<n ; i++ ) ss <<" " << ub[i] ;
				ss << std::endl;
			}
			ss << std::endl;
			this->PrintSelf(ss, itk::Indent(1));
			throw itk::ExceptionObject(__FILE__, __LINE__, "NLOPT Fatal error: " + GetErrorCodeDescription() + ss.str(), "NLOPTOptimizers");
			break;
		default:
			break;
		}
