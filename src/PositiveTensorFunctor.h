...
#ifndef _itkPositiveTensorFunctor_h
#define _itkPositiveTensorFunctor_h 1
...
/************************************************************************************************
 * \class	PositiveSemiDefiniteFunctor
 *
 * \brief	PositiveSemiDefiniteFunctor
 *
...
//    for ( int i=0 ; i<3 ; i++ )	eVal[i] =   (    eVal[i] < minnonzero   ?    minnonzero : eVal[i]  );
...
//#else
  ...
  inline TOutputTensor operator()( const TInputTensor & x )
  {
    ...
    EigenValuesType e;
    TOutputTensor result;
	//double minnonzero = FLT_EPSILON;
    double minnonzero = DBL_MIN;   // Smallest positive number representable.
        ...
        typename TOutputTensorImage::PixelType> >
{
  ...
  typedef TOutputTensorImage OutputImageType;

  /** Method for creation through the object factory. */
  itkNewMacro(Self);

  protected:
    PositiveTensorImageFilter() {}
    virtual ~PositiveTensorImageFilter() {}

  private:
    PositiveTensorImageFilter(const Self&); // purposely not implemented
